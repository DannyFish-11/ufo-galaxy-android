...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v2.5.5 | 2026-03-16 | PR-C1 AIPMessageBuilder 强制 v3：`build()` 的 `includeV3` 参数已移除，v3 信封字段（`version`/`device_id`/`device_type`/`trace_id`/`route_mode`）在所有出站消息中强制存在；新增 `buildCapabilityReport()` 便捷函数，对 `platform`/`supported_actions`/`version` 字段缺失时抛出 `IllegalArgumentException`；LEGACY_TYPE_MAP 仅保留用于解析/兼容路径；新增五大核心消息类型（device_register/heartbeat/capability_report/task_assign/command_result）v3 信封断言测试及 capability_report 缺字段负向测试 |
