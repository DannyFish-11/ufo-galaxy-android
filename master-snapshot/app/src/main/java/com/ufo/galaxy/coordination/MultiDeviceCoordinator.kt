...
class MultiDeviceCoordinator(private val context: Context) {
    ...
    suspend fun sendCommand(
        ...
        try {
            ...
            val response = sendRequest("$serverUrl/api/distributed/send", requestBody)
            
            CommandResult(
                success = response?.optBoolean("success", false) ?: false,
                result = response?.optString("result", ""),
                error = response?.optString("error", "")
            )
        } catch (e: Exception) {
            Log.e(TAG, "Send command failed: ${e.message}")
            CommandResult(success = false, result = "", error = e.message ?: "Unknown error")
        }
    }
     ...
     * 发送消息到指定设备
     */
    suspend fun sendToDevice(targetDeviceId: String, message: String): CommandResult {
        return sendCommand(
            targetDeviceId,
            "notify",
            mapOf("title" to "Galaxy", "message" to message)
        )
    }
     ...
     * 广播消息到所有设备
     */
    suspend fun broadcast(message: String): Boolean = withContext(Dispatchers.IO) {
        try {
            val requestBody = JSONObject().apply {
                put("command", "notify")
            ...
            val response = sendRequest("$serverUrl/api/distributed/broadcast", requestBody)
    ...
    suspend fun discoverDevicesList(): List<DeviceInfo> = withContext(Dispatchers.IO) {
        try {
            val response = sendRequest("$serverUrl/api/distributed/discover", JSONObject())
