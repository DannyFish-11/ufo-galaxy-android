...
class CrossRepoIntegrationValidatorTest {
    ...
    private fun validator(
        ...
        wsUrl: String = "ws://100.0.0.1:9000/ws/device/android-1"
    ) = CrossRepoIntegrationValidator(
        restBaseUrl = "http://100.0.0.1:9000",
        wsUrl = wsUrl,
        httpClient = client
    )
    ...
    fun `all checks pass when server returns 200 and WS URL is valid`() {
        val report = validator(fakeClient(200), "ws://100.0.0.1:9000/ws/device/android-1").validate()
    ...
    fun `WS URL format check passes for ws scheme`() {
        val report = validator(wsUrl = "ws://192.168.1.1:9000/ws/device/android-1").validate()
    ...
    fun `WS URL format check fails for http scheme`() {
        val report = validator(wsUrl = "http://100.0.0.1:9000").validate()
        val wsCheck = report.results.first { it.name == "WS URL format (/ws/device/{device_id})" }
