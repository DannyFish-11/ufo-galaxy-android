...
class AppSettingsTest {
    ...
    fun `galaxyGatewayUrl can be overridden`() {
        val settings = InMemoryAppSettings()
        settings.galaxyGatewayUrl = "ws://192.168.1.42:9000"
        assertEquals("ws://192.168.1.42:9000", settings.galaxyGatewayUrl)
    }

    @Test
    fun `restBaseUrl can be overridden`() {
        val settings = InMemoryAppSettings()
        settings.restBaseUrl = "http://192.168.1.42:9000"
        assertEquals("http://192.168.1.42:9000", settings.restBaseUrl)
    }
    ...
    fun `effectiveGatewayWsUrl uses wss scheme when useTls is true`() {
        val settings = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = true, deviceId = "d1")
        assertEquals("wss://100.64.0.1:9000/ws/device/d1", settings.effectiveGatewayWsUrl())
    }

    @Test
    fun `effectiveRestBaseUrl uses http+host when host is set`() {
        val settings = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = false)
        assertEquals("http://100.64.0.1:9000", settings.effectiveRestBaseUrl())
    }

    @Test
    fun `effectiveRestBaseUrl uses https when useTls is true`() {
        val settings = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = true)
        assertEquals("https://100.64.0.1:9000", settings.effectiveRestBaseUrl())
    }
    ...
    fun `applyGatewayConfig supports structured ws_base and ws_paths`() {
        ...
        val cfg = org.json.JSONObject(
            """{
                "ws_base":"wss://gw.example.com:9000",
                "ws_paths":{"device":"/ws/device/{device_id}"}
            }"""
        )
        settings.applyGatewayConfig(cfg)
        assertEquals("wss://gw.example.com:9000/ws/device/device-777", settings.galaxyGatewayUrl)
    }
