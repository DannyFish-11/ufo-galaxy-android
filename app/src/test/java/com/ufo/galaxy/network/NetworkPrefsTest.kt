...
class NetworkPrefsTest {
    ...
    @Test
    fun `default gatewayPort is 9000`() {
        val s = InMemoryAppSettings()
        assertEquals(SharedPrefsAppSettings.DEFAULT_GATEWAY_PORT, s.gatewayPort)
    }

    @Test
    fun `default useTls is true`() {  // SECURITY-FIX-R5: TLS on by default
        val s = InMemoryAppSettings()
        assertTrue(s.useTls)
    }
    ...
    fun `effectiveGatewayWsUrl uses galaxyGatewayUrl when gatewayHost is blank`() {
        val s = InMemoryAppSettings(galaxyGatewayUrl = "ws://192.168.1.1:9000", deviceId = "device-a")
        // gatewayHost is blank → fall back to galaxyGatewayUrl, then append canonical /ws/device/{device_id}
        assertEquals("ws://192.168.1.1:9000/ws/device/device-a", s.effectiveGatewayWsUrl())
    }

    @Test
    fun `effectiveGatewayWsUrl builds ws URL from host+port when host is set`() {
        val s = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = false, deviceId = "device-a")
        assertEquals("ws://100.64.0.1:9000/ws/device/device-a", s.effectiveGatewayWsUrl())  // explicit false → ws://
    }

    @Test
    fun `effectiveGatewayWsUrl builds wss URL when useTls is true`() {
        val s = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = true, deviceId = "device-a")
        assertEquals("wss://100.64.0.1:9000/ws/device/device-a", s.effectiveGatewayWsUrl())  // explicit true → wss://
    }
    ...
    fun `effectiveRestBaseUrl builds http URL from host+port when host is set`() {
        val s = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = false)
        assertEquals("http://100.64.0.1:9000", s.effectiveRestBaseUrl())  // explicit false → http://
    }

    @Test
    fun `effectiveRestBaseUrl builds https URL when useTls is true`() {
        val s = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = true)
        assertEquals("https://100.64.0.1:9000", s.effectiveRestBaseUrl())  // explicit true → https://
    }
    ...
    fun `allowSelfSigned does not affect URL scheme selection`() {
        ...
        val s = InMemoryAppSettings(gatewayHost = "100.64.0.1", gatewayPort = 9000,
            useTls = false, allowSelfSigned = true)
        // scheme is ws:// because useTls is explicitly false in this test
    ...
    @Test
    fun `DEFAULT_GATEWAY_PORT is 9000`() {
        assertEquals(9000, SharedPrefsAppSettings.DEFAULT_GATEWAY_PORT)
    }
}
...
class NetworkDiagnosticsCheckResultTest {
    ...
    fun `DiagnosticsReport allOk is true when all checks pass`() {
        ...
        val report = NetworkDiagnostics.DiagnosticsReport(
            gatewayHost = "100.64.0.1", gatewayPort = 9000, useTls = true, checks = checks
        )
