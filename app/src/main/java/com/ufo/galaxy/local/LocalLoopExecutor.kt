    ...
    private suspend fun executeViaGoalExecutor(options: LocalLoopOptions): LocalLoopResult =
        withContext(Dispatchers.IO) {
            ...
            val executor = goalExecutor
                ?: return@withContext LocalLoopResult(
                    sessionId = taskId,
                    instruction = options.instruction,
                    status = LocalLoopResult.STATUS_FAILED,
                    stepCount = 0,
                    stopReason = "goal_executor_unavailable",
                    error = "LocalGoalExecutor is null — execution unavailable"
                )
            val goalResult = executor.executeGoal(payload)
