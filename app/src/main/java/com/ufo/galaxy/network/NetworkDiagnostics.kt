...
class NetworkDiagnostics(
    ...
    suspend fun runAll(): DiagnosticsReport {
        ...
        val effectiveDeviceId = deviceId.ifBlank {
        ...
        checks += checkWsHandshake("$wsScheme://$host:$port/ws/device/$effectiveDeviceId")
    ...
    suspend fun checkDns(host: String): CheckResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (result != null) {
                Log.d(TAG, "DNS OK: ${GalaxyWebSocketClient.sanitizeUrlForLogging(host)} → ${result.hostAddress} (${duration}ms)")
            ...
            val duration = System.currentTimeMillis() - t0
            Log.w(TAG, "DNS FAIL: ${GalaxyWebSocketClient.sanitizeUrlForLogging(host)} — ${e.message}")
    ...
    suspend fun checkHttpHealth(restBase: String): CheckResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (response != null) {
                ...
                response.close()
                Log.d(TAG, "HTTP health ${GalaxyWebSocketClient.sanitizeUrlForLogging(url)} → HTTP $code (${duration}ms)")
            ...
            val duration = System.currentTimeMillis() - t0
            Log.w(TAG, "HTTP health FAIL ${GalaxyWebSocketClient.sanitizeUrlForLogging(url)} — ${e.message}")
    ...
    suspend fun checkWsHandshake(wsUrl: String): CheckResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            return@withContext if (result != null) {
                Log.d(TAG, "WS handshake ${GalaxyWebSocketClient.sanitizeUrlForLogging(wsUrl)} → ok=${result.first} detail=${result.second} (${duration}ms)")
            ...
            val duration = System.currentTimeMillis() - t0
            Log.w(TAG, "WS handshake FAIL ${GalaxyWebSocketClient.sanitizeUrlForLogging(wsUrl)} — ${e.message}")
    ...
    suspend fun checkAipPing(restBase: String): CheckResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (response != null) {
                val code = response.code
                response.close()
                Log.d(TAG, "AIP ping ${GalaxyWebSocketClient.sanitizeUrlForLogging(restBase)} → HTTP $code (${duration}ms)")
