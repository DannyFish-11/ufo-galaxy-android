 ...
 *
 * **DEPRECATED**: Use [com.ufo.galaxy.input.InputRouter] for all new routing logic.
 * [InputRouter] is the canonical cross-device uplink path and is the sole routing backbone
 * used by [com.ufo.galaxy.ui.viewmodel.MainViewModel] and
 * [com.ufo.galaxy.service.EnhancedFloatingService]. [InputRouter] carries the following
 * improvements over this class:
 *  - Accepts [GatewayClient] interface for testability.
 *  - Integrates [com.ufo.galaxy.loop.LoopController] for the local execution path.
 *  - Thread-safe double-submit guard via [java.util.concurrent.atomic.AtomicBoolean].
 *  - [com.ufo.galaxy.protocol.TaskSubmitPayload] validation before sending.
 *  - Structured observability via [com.ufo.galaxy.observability.GalaxyLogger].
 *
 * This class is retained for backward compatibility only and will be removed in a future
 * cleanup pass. All cross-device uplink goes through [GalaxyWebSocketClient] via the
 ...
 *    wraps [text] in a [TaskSubmitPayload] envelope and sends it uplink via [GatewayClient].
 ...
 * @param webSocketClient Live gateway client via [GatewayClient] interface.
 ...
 */
@Deprecated(
    message = "Use InputRouter, the canonical cross-device uplink routing backbone.",
    replaceWith = ReplaceWith(
        "InputRouter(settings, webSocketClient, loopController, coroutineScope, onLocalResult, onError)",
        "com.ufo.galaxy.input.InputRouter"
    )
)
class MessageRouter(
