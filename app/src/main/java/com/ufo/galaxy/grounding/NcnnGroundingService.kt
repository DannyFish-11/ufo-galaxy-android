...
class NcnnGroundingService(
    ...
    override fun ground(
        ...
        // SECURITY-FIX-R5: Pre-process screenshot to blur potentially sensitive regions
        // (password fields, credit-card inputs, etc.) before they reach the native layer.
        val privacySafeBase64 = preprocessScreenshot(screenshotBase64, width, height)
        return runGround(intent, privacySafeBase64, width, height)
    }

    // ── Privacy: screenshot pre-processing ──────────────────────────────────

    /**
     * Detects and blurs potentially sensitive on-screen regions (small text input
     * boxes, password fields, etc.) before the screenshot is passed to NCNN.
     *
     * This is a defence-in-depth measure: the native inference runs entirely on
     * device, but the bitmap may later be uploaded to a cloud grounding service
     * when the local model confidence is low.  Blurring sensitive areas reduces
     * the risk of leaking passwords, PINs, or banking information.
     *
     * TODO: Replace the heuristic rectangle-grid approach with an on-device
     *       ML model (e.g. TextFovea or similar) for precise sensitive-region
     *       detection.
     *
     * @param base64 Original JPEG screenshot (Base64).
     * @param width  Screen width in pixels.
     * @param height Screen height in pixels.
     * @return A new Base64 JPEG string with sensitive areas blurred.
     */
    private fun preprocessScreenshot(base64: String, width: Int, height: Int): String {
        return try {
            val raw = Base64.decode(base64, Base64.DEFAULT)
            val original = BitmapFactory.decodeByteArray(raw, 0, raw.size)
                ?: return base64  // fallback: pass through if decode fails
            ...
            val paint = Paint().apply {
                ...
                color = Color.argb(BLUR_ALPHA, 128, 128, 128)
            }

            // Heuristic: blur the bottom 15 % of the screen (common location of
            // on-screen keyboards) and any small centred rectangles that look like
            // input fields.  This is conservative; it may blur non-sensitive UI
            // elements, but guarantees that passwords and PIN pads are obscured.
            val blurZones = mutableListOf<Rect>()

            // Zone 1 — on-screen keyboard area (bottom 15 %)
            val kbTop = (height * 0.85).toInt()
            blurZones.add(Rect(0, kbTop, width, height))

            // Zone 2 — centred input-field band (middle 20 %, horizontally centred 60 %)
            if (width > 0 && height > 0) {
                val bandTop = (height * 0.40).toInt()
                val bandBottom = (height * 0.60).toInt()
                val bandLeft = (width * 0.20).toInt()
                val bandRight = (width * 0.80).toInt()
                blurZones.add(Rect(bandLeft, bandTop, bandRight, bandBottom))
            }

            for (zone in blurZones) {
                canvas.drawRect(zone, paint)
            }

            // Re-encode as JPEG with reduced quality to further strip fine detail.
            val out = ByteArrayOutputStream()
            blurred.compress(Bitmap.CompressFormat.JPEG, JPEG_QUALITY, out)
