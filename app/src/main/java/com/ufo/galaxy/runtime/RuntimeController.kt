...
class RuntimeController(
    ...
    /**
     * ROUND-2-FIX: Timestamp (SystemClock.elapsedRealtime) when [_startInProgress] was last
     * set to true. Used to detect a stale lock left behind by a process kill or coroutine
     * cancellation that did not run the finally block. If the lock is held longer than
     * [START_LOCK_TIMEOUT_MS] we treat it as expired and allow a new start attempt.
     *
     * Uses [SystemClock.elapsedRealtime] (monotonic clock) instead of
     * [System.currentTimeMillis] so device time changes cannot cause a false
     * stale-lock detection.
     */
    @Volatile
    private var _startLockAcquiredAtMs: Long = 0L
    ...
    suspend fun start(): Boolean {
        ...
        // ROUND-2-FIX: Add timeout-based stale-lock protection so a process kill that
        // bypasses the finally block cannot permanently block future start attempts.
        val lockAlreadySet = _startInProgress.get()
        if (lockAlreadySet) {
            val heldForMs = SystemClock.elapsedRealtime() - _startLockAcquiredAtMs
            if (heldForMs < START_LOCK_TIMEOUT_MS) {
            ...
            // Stale lock detected — force-reset and proceed.
            Log.w(TAG, "[RUNTIME] Start lock stale (${heldForMs}ms) — forcing reset and proceeding")
            GalaxyLogger.log(TAG, mapOf("event" to "runtime_start_stale_lock_reset", "held_for_ms" to heldForMs))
            _startInProgress.set(false)
        }
        if (!_startInProgress.compareAndSet(false, true)) {
            // Another thread raced and won — return current state.
            return _state.value == RuntimeState.Active
        }
        _startLockAcquiredAtMs = SystemClock.elapsedRealtime()
        ...
        // ROUND-3-FIX: AtomicBoolean guard prevents double-resume if both onConnected
        // and onError fire (or any other combination), which would throw
        // IllegalStateException: "Already resumed".
        val connected = suspendCancellableCoroutine<Boolean> { cont ->
            val resumed = AtomicBoolean(false)

            // ROUND-3-FIX: If the WS client is already connected, resume immediately
            // so the coroutine does not hang forever waiting for a listener callback
            // that will never arrive (connect() returns early when already connected).
            if (webSocketClient.isConnected()) {
                if (resumed.compareAndSet(false, true)) {
                    Log.i(TAG, "[RUNTIME] WS already connected — resuming immediately")
                    GalaxyLogger.log(TAG, mapOf("event" to "runtime_registered_already_connected"))
                    cont.resume(true)
                }
            ...
            cont.invokeOnCancellation {
                ...
                // Ensure we don't leave the coroutine suspended if cancellation races
    ...
    private fun isGatewayConfigured(): Boolean {
        ...
        // Reject the default placeholder pattern (e.g. "ws://100.x.x.x:9000").
    ...
    private fun transitionState(
        ...
        val event: RuntimeLifecycleTransitionEvent = if (isGoverned && !mismatch) {
            ...
            val reason = when {
                ...
                mismatch    -> "prior_state_mismatch_expected_${expectedFrom?.wireLabel ?: "unknown"}"
                else        -> "unexpected"
            }
            Log.w(
                TAG,
