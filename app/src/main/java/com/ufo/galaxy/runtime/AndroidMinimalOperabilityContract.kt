...
object AndroidMinimalOperabilityContract {
    ...
    enum class OperabilityPathStep(
         ...
         *
         * 前提：V2 服务已运行；已知 V2 宿主机 IP 和端口（默认 8765）。
        ...
        CONFIGURE_V2_SERVER(
            ...
            description = "设置 V2 gateway WebSocket URL（ws://<host>:8765/ws/device/<id>）并启用 crossDeviceEnabled。",
    ...
    enum class PathBlockCondition(
        ...
        GATEWAY_URL_IS_PLACEHOLDER(
            ...
            description = "config.properties 中的默认 URL（ws://100.x.x.x:8765）是占位符。必须在 App 设置中替换为真实 V2 地址才能建立连接。",
    ...
    enum class FailureDiagnosticKind(
        ...
        CONNECT_FAILED_INVALID_GATEWAY_URL(
            ...
            actionableHint = "在 App ⚙ 设置中将 Gateway URL 更新为 ws://<V2宿主IP>:8765；确认 V2 服务正在运行。"
        ),
        ...
        CONNECT_FAILED_HOST_UNREACHABLE(
            ...
            affectedStep = OperabilityPathStep.ESTABLISH_WS_CONNECTION,
            actionableHint = "确认 V2 服务已运行（python main.py --host <ip> --port 8765）；检查网络路径（Tailscale/LAN）。"
        ),
