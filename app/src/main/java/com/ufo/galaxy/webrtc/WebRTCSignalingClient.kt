 ...
 *                         callbacks for full-chain log correlation.
...
class WebRTCSignalingClient(
    ...
    private val wsListener = object : WebSocketListener() {
        override fun onOpen(ws: WebSocket, response: Response) {
            Log.i(TAG, "[trace=$traceId] Signaling WS connected: $url")
            isConnected = true
            webSocket = ws
            onConnected()
        }
        ...
        override fun onMessage(ws: WebSocket, text: String) {
            Log.d(TAG, "[trace=$traceId] Signaling message received: $text")
            val message = SignalingMessage.fromJsonString(text)
            if (message == null) {
                Log.w(TAG, "[trace=$traceId] Failed to parse signaling message: $text")
                return
            }
        ...
        override fun onClosing(ws: WebSocket, code: Int, reason: String) {
            Log.i(TAG, "[trace=$traceId] Signaling WS closing: $code $reason")
            ws.close(1000, null)
        }

        override fun onClosed(ws: WebSocket, code: Int, reason: String) {
            Log.i(TAG, "[trace=$traceId] Signaling WS closed: $code $reason")
            isConnected = false
            webSocket = null
            onDisconnected()
        }
        ...
        override fun onFailure(ws: WebSocket, t: Throwable, response: Response?) {
            val msg = "[trace=$traceId] Signaling WS failure: ${t.message}"
            Log.e(TAG, msg, t)
            isConnected = false
            webSocket = null
            onError(msg)
        ...
        when (message.type) {
            SignalingMessage.TYPE_ICE_CANDIDATE -> {
                ...
                if (candidates.isNotEmpty()) {
                    ...
                    Log.d(TAG, "[trace=$traceId] Applied $applied/${candidates.size} legacy candidate(s)")
                } else {
                    Log.w(TAG, "[trace=$traceId] ice_candidate message had no candidate data")
                }
            }
            SignalingMessage.TYPE_ICE_CANDIDATES -> {
                ...
                message.turnConfig?.let { tc ->
                    lastTurnConfig = tc
                    Log.i(TAG, "[trace=$traceId] Received TURN config: ${tc.urls}")
                }
                val candidates = message.allCandidates
                if (candidates.isNotEmpty()) {
                    val applied = candidateManager.addCandidates(candidates)
                    Log.i(TAG, "[trace=$traceId] Applied $applied/${candidates.size} candidate(s) " +
                            "(relay=${candidateManager.getTurnCandidates().size})")
                } else {
                    Log.w(TAG, "[trace=$traceId] ice_candidates message contained no candidates")
                }
            }
            SignalingMessage.TYPE_ERROR -> {
                val errMsg = "[trace=$traceId] Gateway error: ${message.error ?: "unknown"}"
                Log.e(TAG, errMsg)
                onError(errMsg)
            }
            else -> {
     ...
     */
    fun connect() {
        Log.i(TAG, "[trace=$traceId] Connecting to WebRTC signaling endpoint: $url")
    ...
    fun send(message: SignalingMessage) {
        ...
        Log.d(TAG, "[trace=$traceId] Sending signaling message type=${message.type}")
        webSocket?.send(json) ?: Log.w(TAG, "[trace=$traceId] Cannot send: signaling WS not connected")
    }
    ...
    fun sendJson(json: JSONObject) {
        Log.d(TAG, "[trace=$traceId] Sending raw signaling JSON: ${json.optString("type")}")
        webSocket?.send(json.toString()) ?: Log.w(TAG, "[trace=$traceId] Cannot send: signaling WS not connected")
    }
    ...
    fun triggerTurnFallback() {
        Log.w(TAG, "[trace=$traceId] ICE connectivity failed – triggering TURN fallback")
     ...
     */
    fun disconnect() {
        Log.i(TAG, "[trace=$traceId] Disconnecting signaling WS")
