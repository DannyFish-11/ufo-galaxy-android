...
class EnhancedFloatingService : Service() {
    ...
    companion object {
        ...
        private val NOTIFICATION_ID = ("EnhancedFloatingService".hashCode() and 0x7FFFFFFF) % 10000 + 1000
     ...
     * C2-FIX: Default dispatcher changed to IO to avoid blocking the main thread for network/WebSocket ops.
     * UI updates use withContext(Dispatchers.Main) where needed. */
    ...
    private val inputRouter: InputRouter by lazy {
        InputRouter(
            ...
            webSocketClient = AipTransportManager.getInstance(),
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        ...
        // A4-FIX: Clean up floating view when the task is removed (swiped from recents)
        Log.i(TAG, "Task removed — cleaning up floating view (A4-FIX)")
        removeFloatingView()
    }
    ...
    override fun onDestroy() {
        ...
        // A4-FIX: Defensive cleanup with try-finally to ensure view is always removed
        // even if intermediate steps throw, preventing WindowManager leaks.
        try {
            serviceScope.cancel()
        } catch (e: Exception) {
            Log.w(TAG, "Exception cancelling serviceScope: ${e.message}")
        }
        try {
            unregisterReceiver(wakeUpReceiver)
        } catch (e: Exception) {
            Log.w(TAG, "Exception unregistering receiver: ${e.message}")
        }
        edgeTrigger?.stop()
        try {
            ...
            Log.w(TAG, "Exception removing WS listener: ${e.message}")
        } finally {
            removeFloatingView()
            floatingView = null
            Log.i(TAG, "Floating view removed and reference cleared (A4-FIX)")
        }
    }
    ...
    private fun createFloatingView() {
        floatingView = LinearLayout(this).apply {
            ...
            statusText = TextView(context).apply {
                ...
                contentDescription = "Galaxy状态: ${text}"  // C14-FIX: Accessibility label
            ...
            chatContainer = LinearLayout(context).apply {
                ...
                val inputArea = LinearLayout(context).apply {
                    ...
                    voiceButton = ImageButton(context).apply {
                        contentDescription = "语音输入"  // C14-FIX: Accessibility label
                    ...
                    inputField = EditText(context).apply {
                        hint = "输入消息..."
                        contentDescription = "消息输入框"  // C14-FIX: Accessibility label
                        setTextColor(0xFFFFFFFF.toInt())
                    ...
                    sendButton = ImageButton(context).apply {
                        contentDescription = "发送消息"  // C14-FIX: Accessibility label
