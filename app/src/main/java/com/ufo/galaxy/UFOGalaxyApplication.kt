...
class UFOGalaxyApplication : Application() {
    ...
    companion object {
        ...
        /**
         * WeakReference wrapper for the Application instance to prevent memory leaks (A1-FIX).
        ...
        var instance: UFOGalaxyApplication? = null
            private set
        
        // 全局 WebSocket 客户端 — WeakReference to prevent GC root chain (A1-FIX)
        private var webSocketClientRef: WeakReference<GalaxyWebSocketClient> = WeakReference(null)
        ...
        // C9-FIX: Canonical three-phase state machine (SILENT / LIMINAL / MANIFEST).
        // Shared between GalaxyConnectionService (writes phase transitions) and
        // MainViewModel (reads currentPhase for UI visualization).
        val phaseStateMachine: PhaseStateMachine = PhaseStateMachine()

        /**
         * SECURITY-FIX: Application-wide CoroutineScope with SupervisorJob for structured concurrency.
         * All application-level coroutines should use this scope instead of creating throwaway scopes.
         * Cancelled in [onTerminate] to prevent coroutine leaks.
         */
        val applicationScope: CoroutineScope = CoroutineScope(
            Dispatchers.IO + SupervisorJob() + kotlinx.coroutines.CoroutineName("UFOGalaxyApplication")
        )
    ...
    override fun onCreate() {
        ...
        // SECURITY-FIX: APK signature verification (anti-tampering).
        // Must run BEFORE any sensitive initialization so that a repackaged
        // app cannot execute our code with a forged signature.
        if (!com.ufo.galaxy.security.SignatureVerifier.verifyAppSignature(this)) {
            throw SecurityException("Application signature verification failed. App cannot start.")
        }

        instance = this
        instanceRef = WeakReference(this)
        Log.d(TAG, "Application instance set (DEBUG=${BuildConfig.DEBUG})")

        // A1-FIX: Set global uncaught exception handler to catch and log crashes (B8-FIX)
        Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
            Log.e(TAG, "CRASH on thread ${thread.name}: ${throwable.message}", throwable)
            GalaxyLogger.log("CRASH", mapOf(
        ...
        // SECURITY-FIX: Use applicationScope instead of throwaway CoroutineScope for lifecycle-bound coroutines.
    ...
    override fun onTerminate() {
        super.onTerminate()
        // A1-FIX: Explicitly release all static references to prevent memory leaks.
        // onTerminate() is not guaranteed on real devices, but is called in tests/emulators.
        ...
        instance = null
        instanceRef.clear()
        webSocketClientRef.clear()
        Log.i(TAG, "UFO Galaxy Application 终止 — all static references cleared (CRITICAL-6)")
    }
    ...
    private fun initConfig() {
        ...
        // SECURITY: Log only sanitized URL — never log the full server URL with possible credentials.
        val sanitizedConfigUrl = sanitizeUrlForLogging(appConfig.serverUrl)
        Log.d(TAG, "Config loaded: serverUrl=$sanitizedConfigUrl crossDevice=${appSettings.crossDeviceEnabled}")
    }
    ...
    private fun initWebSocketClient() {
        ...
        // All GatewayClient consumers now go through AipTransportManager.
        AipTransportManager.getInstance().registerAdapter("websocket", client)
        client.setDeviceMetadata(appSettings.toMetadataMap())
        // SECURITY: Log only sanitized URL (protocol + host only) — never log full URL
        // which may contain gateway tokens or other sensitive query parameters.
        val sanitizedUrl = sanitizeUrlForLogging(wsUrl)
        Log.d(TAG, "WebSocket client initialized: url=$sanitizedUrl allowSelfSigned=${appSettings.effectiveAllowSelfSigned()} (offlineQueue restored size=${offlineQueue.size})")
    }
    ...
    private fun initAgentRuntimeBridge() {
        agentRuntimeBridge = AgentRuntimeBridge(
            gatewayClient = AipTransportManager.getInstance(),
            settings = appSettings,
