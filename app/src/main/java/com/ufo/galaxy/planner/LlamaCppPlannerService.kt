...
class LlamaCppPlannerService(
    ...
    private companion object {
        ...
        // ROUND-2-FIX: Reuse a single Gson instance across all parsing calls.
        // Gson is thread-safe, so a single singleton avoids repeated allocation.
    ...
    // ROUND-3-FIX: @Synchronized prevents TOCTOU race between isModelLoaded() check and
    // nativeHandle usage. Without this, unloadModel() could interleave and zero the handle
    // after the check passes but before nativeCompletion() runs, causing a JNI crash.
    @Synchronized
    override fun plan(
        goal: String,
    ...
    // ROUND-3-FIX: Validate nativeHandle at entry so that unloadModel() interleaved after
    // the isModelLoaded() check cannot pass an invalid (0 or stale) handle to JNI,
    // which would crash the JVM.  @Synchronized ensures this check and the native call
    // are atomic with respect to loadModel() / unloadModel().
    @Synchronized
    private fun runCompletion(prompt: String, maxTokens: Int): String? {
        if (nativeHandle <= 0L) {
            Log.e(TAG, "runCompletion: nativeHandle is invalid ($nativeHandle) — model not loaded or already unloaded")
            return null
        }
        return try {
